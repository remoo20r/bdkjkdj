...
/// Bright silver used for the focus ring + halo (a premium "chrome" edge).
const Color _kSilver = Color(0xFFE6E9F0);

/// Light red used for the focused-item fill tint and outer bloom.
const Color _kFocusRed = Color(0xFFFF5A6A);
            ...
            child: GestureDetector(
              ...
              child: AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  final glowAlpha = 0.40 + (t * 0.35); // 0.40 -> 0.75
                  final glowBlur = 16.0 + (t * 12.0); // 16 -> 28
                  final ringWidth = focused ? 3.5 : (_hovered ? 2.0 : 0.0);

                  return Container(
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(widget.borderRadius),
                      // Light-red translucent fill behind the focused item
                      // (replaces the old yellow/gold tint) so the highlight
                      // reads warm without hiding the content.
                      color: focused
                          ? _kFocusRed.withValues(alpha: 0.16 + t * 0.06)
                          : null,
                      border: Border.all(
                        // Focus = a thick bright SILVER ring, unmistakable and
                        // premium; hover is only a soft silver hint.
                        color: focused
                            ? _kSilver
                            : (_hovered ? _kSilver.withValues(alpha: 0.5) : Colors.transparent),
                        width: ringWidth,
                      ),
                      boxShadow: focused
                          ? [
                              // Silver halo — the main "you are here" marker.
                              BoxShadow(
                                color: _kSilver.withValues(alpha: glowAlpha),
                                blurRadius: glowBlur,
                                spreadRadius: 1 + t,
                              ),
                              // Soft light-red outer bloom for warmth.
                              BoxShadow(
                                color: _kFocusRed.withValues(alpha: glowAlpha * 0.45),
                                blurRadius: glowBlur * 1.7,
                                spreadRadius: 0,
                              ),
                            ]
                          : null,
